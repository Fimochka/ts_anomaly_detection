...
## Дизайн ML системы - ts_anomaly_detection

### 1. Цели и предпосылки 
#### 1.1. Зачем идем в разработку продукта?  

- Бизнес-цель: продукт позволит организации из коробки развернуть и настроить мониторинг временных рядов и обнаружение аномалий с помощью различных методов и моделей
- Почему станет лучше, чем сейчас, от использования ML: задача поиска мониторинга на практике отнимает много ресурсов, т.к. требуется целый набор инструментов для мониторинга, регулярных задач и отдельно нужно исследовать и внедрять поиск аномалий (это также требует определенных компетенций и времени). Наш сервис позволит решать все эти задачи в одном месте и с минимальными временными и человеческими затратами (при этом требования к компетенциям сотрудников также будут снижены).
...
- Что будем считать успехом итерации с точки зрения бизнеса: уменьшение времени на настройку мониторинга и поиска аномалий (с оповещениями) в компании. 
...
- Что мы ожидаем от конкретной итерации: сервис, который можно развернуть с помощью docker-compose и настроить полномасштабный мониторинг и поиск аномалий.
- Описание бизнес-процесса пилота:
  - подключить источники данных (CH, Postgresql, hive, etc)
  - добавить временные ряды (метрики) через единый удобный интерфейс
  - для любой метрики выбрать алгоритм и выбрать предиктивную модель, которая будет обучаться на исторических данных с учетом всех особенностей (тренд, сезонность),
  - добавить внешние факторы (если нужно)
...
- Что считаем успешным пилотом? Критерии успеха и возможные пути развития проекта: использование инструмента в реальной жизни (на работе)
